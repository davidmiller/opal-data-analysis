...
OPALDATA <- "~/src/ohc/data/emmanuel.wey.2020-06-10/"
...
field <- function(fieldname, data){
  ...
  contents <- as.data.frame(without.field)
  names(contents) <- c(fieldname)
  return(contents)
}

plot.field <- function(fieldname, data, filename){
  contents <- field(fieldname, data)
  aggregated <- as.data.frame(table(contents))
  names(aggregated) <- c("Thing", "Frequency")
  
  bar_plot(aggregated, aes(x=Thing, y=Frequency, fill=Thing), fieldname, filename=filename)
}
